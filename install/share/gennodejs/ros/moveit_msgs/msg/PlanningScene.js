...
class PlanningScene {
  ...
  static serialize(obj, bufferInfo) {
    ...
    bufferInfo = RobotState.serialize(obj.robot_state, bufferInfo);
    // Serialize message field [robot_model_name]
    bufferInfo = _serializer.string(obj.robot_model_name, bufferInfo);
    // Serialize the length for message field [fixed_frame_transforms]
    bufferInfo = _serializer.uint32(obj.fixed_frame_transforms.length, bufferInfo);
    ...
    bufferInfo = AllowedCollisionMatrix.serialize(obj.allowed_collision_matrix, bufferInfo);
    // Serialize the length for message field [link_padding]
    bufferInfo = _serializer.uint32(obj.link_padding.length, bufferInfo);
    // Serialize message field [link_padding]
    obj.link_padding.forEach((val) => {
      bufferInfo = LinkPadding.serialize(val, bufferInfo);
    });
    // Serialize the length for message field [link_scale]
    bufferInfo = _serializer.uint32(obj.link_scale.length, bufferInfo);
    // Serialize message field [link_scale]
    obj.link_scale.forEach((val) => {
      bufferInfo = LinkScale.serialize(val, bufferInfo);
    });
    // Serialize the length for message field [object_colors]
    bufferInfo = _serializer.uint32(obj.object_colors.length, bufferInfo);
    // Serialize message field [object_colors]
    obj.object_colors.forEach((val) => {
      bufferInfo = ObjectColor.serialize(val, bufferInfo);
    });
    // Serialize message field [world]
    bufferInfo = PlanningSceneWorld.serialize(obj.world, bufferInfo);
    // Serialize message field [is_diff]
    bufferInfo = _serializer.bool(obj.is_diff, bufferInfo);
    return bufferInfo;
  }
  ...
  static deserialize(buffer) {
    ...
    data.robot_state = tmp.data;
    buffer = tmp.buffer;
    // Deserialize message field [robot_model_name]
    tmp = _deserializer.string(buffer);
    data.robot_model_name = tmp.data;
    buffer = tmp.buffer;
    ...
    for (let i = 0; i < len; ++i) {
      ...
      data.fixed_frame_transforms[i] = tmp.data;
      buffer = tmp.buffer;
    }
    // Deserialize message field [allowed_collision_matrix]
    tmp = AllowedCollisionMatrix.deserialize(buffer);
    data.allowed_collision_matrix = tmp.data;
    buffer = tmp.buffer;
    ...
    for (let i = 0; i < len; ++i) {
      tmp = LinkPadding.deserialize(buffer);
      data.link_padding[i] = tmp.data;
      buffer = tmp.buffer;
    }
    ...
    for (let i = 0; i < len; ++i) {
      tmp = LinkScale.deserialize(buffer);
      data.link_scale[i] = tmp.data;
      buffer = tmp.buffer;
    }
    ...
    for (let i = 0; i < len; ++i) {
      tmp = ObjectColor.deserialize(buffer);
      data.object_colors[i] = tmp.data;
      buffer = tmp.buffer;
    }
    // Deserialize message field [world]
    tmp = PlanningSceneWorld.deserialize(buffer);
    data.world = tmp.data;
    buffer = tmp.buffer;
    // Deserialize message field [is_diff]
    tmp = _deserializer.bool(buffer);
    data.is_diff = tmp.data;
    buffer = tmp.buffer;
    return {
      data: data,
      buffer: buffer
