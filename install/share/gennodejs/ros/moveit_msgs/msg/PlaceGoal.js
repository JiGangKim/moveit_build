...
class PlaceGoal {
  ...
  static serialize(obj, bufferInfo) {
    ...
    bufferInfo = _serializer.string(obj.attached_object_name, bufferInfo);
    // Serialize the length for message field [place_locations]
    bufferInfo = _serializer.uint32(obj.place_locations.length, bufferInfo);
    // Serialize message field [place_locations]
    obj.place_locations.forEach((val) => {
      bufferInfo = PlaceLocation.serialize(val, bufferInfo);
    });
    // Serialize message field [place_eef]
    bufferInfo = _serializer.bool(obj.place_eef, bufferInfo);
  ...
  static deserialize(buffer) {
    ...
    tmp = _deserializer.string(buffer);
    data.attached_object_name = tmp.data;
    buffer = tmp.buffer;
    ...
    for (let i = 0; i < len; ++i) {
      tmp = PlaceLocation.deserialize(buffer);
      data.place_locations[i] = tmp.data;
      buffer = tmp.buffer;
    }
    // Deserialize message field [place_eef]
    tmp = _deserializer.bool(buffer);
    data.place_eef = tmp.data;
    buffer = tmp.buffer;
