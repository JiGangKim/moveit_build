...
class GetPositionFKRequest {
  ...
  static serialize(obj, bufferInfo) {
    ...
    // Serialize the length for message field [fk_link_names]
    bufferInfo = _serializer.uint32(obj.fk_link_names.length, bufferInfo);
    // Serialize message field [fk_link_names]
    obj.fk_link_names.forEach((val) => {
  ...
  static deserialize(buffer) {
    ...
    let tmp;
    let len;
    let data = new GetPositionFKRequest();
    // Deserialize message field [header]
    tmp = std_msgs.msg.Header.deserialize(buffer);
    data.header = tmp.data;
    buffer = tmp.buffer;
    // Deserialize array length for message field [fk_link_names]
    tmp = _deserializer.uint32(buffer);
    len = tmp.data;
    buffer = tmp.buffer;
    // Deserialize message field [fk_link_names]
    data.fk_link_names = new Array(len);
    for (let i = 0; i < len; ++i) {
      tmp = _deserializer.string(buffer);
      data.fk_link_names[i] = tmp.data;
      buffer = tmp.buffer;
    }
...
class GetPositionFKResponse {
  ...
  static serialize(obj, bufferInfo) {
    ...
    bufferInfo = _serializer.uint32(obj.pose_stamped.length, bufferInfo);
  ...
  static deserialize(buffer) {
    ...
    let tmp;
    let len;
    let data = new GetPositionFKResponse();
