...
class GraspPlanningRequest {
  ...
  static serialize(obj, bufferInfo) {
    ...
    bufferInfo = _serializer.string(obj.arm_name, bufferInfo);
    // Serialize message field [target]
    bufferInfo = GraspableObject.serialize(obj.target, bufferInfo);
    // Serialize message field [collision_object_name]
    bufferInfo = _serializer.string(obj.collision_object_name, bufferInfo);
    // Serialize message field [collision_support_surface_name]
    bufferInfo = _serializer.string(obj.collision_support_surface_name, bufferInfo);
    // Serialize the length for message field [grasps_to_evaluate]
    bufferInfo = _serializer.uint32(obj.grasps_to_evaluate.length, bufferInfo);
    ...
    bufferInfo = _serializer.uint32(obj.movable_obstacles.length, bufferInfo);
    // Serialize message field [movable_obstacles]
    obj.movable_obstacles.forEach((val) => {
      bufferInfo = GraspableObject.serialize(val, bufferInfo);
    });
    return bufferInfo;
  }
  ...
  static deserialize(buffer) {
    ...
    let tmp;
    let len;
    let data = new GraspPlanningRequest();
    // Deserialize message field [arm_name]
    tmp = _deserializer.string(buffer);
    data.arm_name = tmp.data;
    buffer = tmp.buffer;
    // Deserialize message field [target]
    tmp = GraspableObject.deserialize(buffer);
    data.target = tmp.data;
    buffer = tmp.buffer;
    // Deserialize message field [collision_object_name]
    tmp = _deserializer.string(buffer);
    data.collision_object_name = tmp.data;
    buffer = tmp.buffer;
    // Deserialize message field [collision_support_surface_name]
    tmp = _deserializer.string(buffer);
    data.collision_support_surface_name = tmp.data;
    buffer = tmp.buffer;
    ...
    for (let i = 0; i < len; ++i) {
      ...
      data.grasps_to_evaluate[i] = tmp.data;
      buffer = tmp.buffer;
    }
    ...
    for (let i = 0; i < len; ++i) {
      tmp = GraspableObject.deserialize(buffer);
      data.movable_obstacles[i] = tmp.data;
      buffer = tmp.buffer;
    }
    return {
      data: data,
...
class GraspPlanningResponse {
  ...
  static deserialize(buffer) {
    ...
    let tmp;
    let len;
    let data = new GraspPlanningResponse();
