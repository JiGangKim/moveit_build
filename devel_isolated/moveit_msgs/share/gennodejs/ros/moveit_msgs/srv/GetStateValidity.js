...
class GetStateValidityRequest {
  ...
  static deserialize(buffer) {
    ...
    let tmp;
    let len;
    let data = new GetStateValidityRequest();
...
class GetStateValidityResponse {
  ...
  static serialize(obj, bufferInfo) {
    ...
    bufferInfo = _serializer.uint32(obj.contacts.length, bufferInfo);
    // Serialize message field [contacts]
    obj.contacts.forEach((val) => {
      bufferInfo = ContactInformation.serialize(val, bufferInfo);
    });
    // Serialize the length for message field [cost_sources]
    bufferInfo = _serializer.uint32(obj.cost_sources.length, bufferInfo);
    // Serialize message field [cost_sources]
    obj.cost_sources.forEach((val) => {
      bufferInfo = CostSource.serialize(val, bufferInfo);
    });
    // Serialize the length for message field [constraint_result]
    bufferInfo = _serializer.uint32(obj.constraint_result.length, bufferInfo);
    // Serialize message field [constraint_result]
    obj.constraint_result.forEach((val) => {
      bufferInfo = ConstraintEvalResult.serialize(val, bufferInfo);
    });
    return bufferInfo;
  }
  ...
  static deserialize(buffer) {
    ...
    let tmp;
    let len;
    let data = new GetStateValidityResponse();
    ...
    for (let i = 0; i < len; ++i) {
      tmp = ContactInformation.deserialize(buffer);
      data.contacts[i] = tmp.data;
      buffer = tmp.buffer;
    }
    ...
    for (let i = 0; i < len; ++i) {
      tmp = CostSource.deserialize(buffer);
      data.cost_sources[i] = tmp.data;
      buffer = tmp.buffer;
    }
    ...
    for (let i = 0; i < len; ++i) {
      tmp = ConstraintEvalResult.deserialize(buffer);
      data.constraint_result[i] = tmp.data;
      buffer = tmp.buffer;
    }
    return {
      data: data,
