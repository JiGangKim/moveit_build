...
class DisplayRobotState {
  ...
  static serialize(obj, bufferInfo) {
    ...
    bufferInfo = RobotState.serialize(obj.state, bufferInfo);
    // Serialize the length for message field [highlight_links]
    bufferInfo = _serializer.uint32(obj.highlight_links.length, bufferInfo);
  ...
  static deserialize(buffer) {
    ...
    let tmp;
    let len;
    let data = new DisplayRobotState();
    ...
    for (let i = 0; i < len; ++i) {
      ...
      data.highlight_links[i] = tmp.data;
      buffer = tmp.buffer;
    }
    return {
      data: data,
