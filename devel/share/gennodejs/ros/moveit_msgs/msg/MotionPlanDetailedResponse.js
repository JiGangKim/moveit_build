...
class MotionPlanDetailedResponse {
  ...
  static serialize(obj, bufferInfo) {
    ...
    bufferInfo = RobotState.serialize(obj.trajectory_start, bufferInfo);
    ...
    bufferInfo = _serializer.uint32(obj.trajectory.length, bufferInfo);
    // Serialize message field [trajectory]
    obj.trajectory.forEach((val) => {
      bufferInfo = RobotTrajectory.serialize(val, bufferInfo);
    });
    // Serialize the length for message field [description]
    bufferInfo = _serializer.uint32(obj.description.length, bufferInfo);
    // Serialize message field [description]
    obj.description.forEach((val) => {
      bufferInfo = _serializer.string(val, bufferInfo);
    });
    // Serialize the length for message field [processing_time]
    bufferInfo = _serializer.uint32(obj.processing_time.length, bufferInfo);
    // Serialize message field [processing_time]
    obj.processing_time.forEach((val) => {
  ...
  static deserialize(buffer) {
    ...
    for (let i = 0; i < len; ++i) {
      tmp = _deserializer.string(buffer);
      data.description[i] = tmp.data;
      buffer = tmp.buffer;
    }
    // Deserialize array length for message field [processing_time]
    tmp = _deserializer.uint32(buffer);
    len = tmp.data;
    buffer = tmp.buffer;
    // Deserialize message field [processing_time]
    data.processing_time = new Array(len);
    for (let i = 0; i < len; ++i) {
      tmp = _deserializer.float64(buffer);
      data.processing_time[i] = tmp.data;
      buffer = tmp.buffer;
    }
