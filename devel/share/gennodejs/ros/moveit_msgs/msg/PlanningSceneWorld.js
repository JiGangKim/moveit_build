...
class PlanningSceneWorld {
  ...
  static serialize(obj, bufferInfo) {
    ...
    bufferInfo = _serializer.uint32(obj.collision_objects.length, bufferInfo);
    // Serialize message field [collision_objects]
    obj.collision_objects.forEach((val) => {
      bufferInfo = CollisionObject.serialize(val, bufferInfo);
    });
    // Serialize message field [octomap]
    bufferInfo = octomap_msgs.msg.OctomapWithPose.serialize(obj.octomap, bufferInfo);
    return bufferInfo;
  }
  ...
  static deserialize(buffer) {
    ...
    let tmp;
    let len;
    let data = new PlanningSceneWorld();
    ...
    for (let i = 0; i < len; ++i) {
      ...
      data.collision_objects[i] = tmp.data;
      buffer = tmp.buffer;
    }
    // Deserialize message field [octomap]
    tmp = octomap_msgs.msg.OctomapWithPose.deserialize(buffer);
    data.octomap = tmp.data;
    buffer = tmp.buffer;
    return {
      data: data,
      buffer: buffer
