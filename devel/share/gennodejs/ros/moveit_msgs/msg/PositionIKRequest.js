...
class PositionIKRequest {
  ...
  static serialize(obj, bufferInfo) {
    ...
    bufferInfo = Constraints.serialize(obj.constraints, bufferInfo);
    // Serialize message field [avoid_collisions]
    bufferInfo = _serializer.bool(obj.avoid_collisions, bufferInfo);
    // Serialize message field [ik_link_name]
    bufferInfo = _serializer.string(obj.ik_link_name, bufferInfo);
    // Serialize message field [pose_stamped]
    bufferInfo = geometry_msgs.msg.PoseStamped.serialize(obj.pose_stamped, bufferInfo);
    // Serialize the length for message field [ik_link_names]
    bufferInfo = _serializer.uint32(obj.ik_link_names.length, bufferInfo);
    // Serialize message field [ik_link_names]
    obj.ik_link_names.forEach((val) => {
      bufferInfo = _serializer.string(val, bufferInfo);
    });
    // Serialize the length for message field [pose_stamped_vector]
    bufferInfo = _serializer.uint32(obj.pose_stamped_vector.length, bufferInfo);
    ...
    bufferInfo = _serializer.int32(obj.attempts, bufferInfo);
    return bufferInfo;
  }
  ...
  static deserialize(buffer) {
    ...
    let tmp;
    let len;
    let data = new PositionIKRequest();
    ...
    for (let i = 0; i < len; ++i) {
      tmp = geometry_msgs.msg.PoseStamped.deserialize(buffer);
      data.pose_stamped_vector[i] = tmp.data;
      buffer = tmp.buffer;
    }
