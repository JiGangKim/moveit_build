...
class GraspableObject {
  ...
  static serialize(obj, bufferInfo) {
    ...
    bufferInfo = _serializer.string(obj.reference_frame_id, bufferInfo);
    // Serialize the length for message field [potential_models]
    bufferInfo = _serializer.uint32(obj.potential_models.length, bufferInfo);
    // Serialize message field [potential_models]
    obj.potential_models.forEach((val) => {
      bufferInfo = household_objects_database_msgs.msg.DatabaseModelPose.serialize(val, bufferInfo);
    });
    // Serialize message field [cluster]
    bufferInfo = sensor_msgs.msg.PointCloud.serialize(obj.cluster, bufferInfo);
    // Serialize message field [region]
    bufferInfo = SceneRegion.serialize(obj.region, bufferInfo);
    // Serialize message field [collision_name]
    bufferInfo = _serializer.string(obj.collision_name, bufferInfo);
    return bufferInfo;
  }
  ...
  static deserialize(buffer) {
    ...
    let tmp;
    let len;
    let data = new GraspableObject();
    // Deserialize message field [reference_frame_id]
    tmp = _deserializer.string(buffer);
    data.reference_frame_id = tmp.data;
    buffer = tmp.buffer;
    ...
    for (let i = 0; i < len; ++i) {
      tmp = household_objects_database_msgs.msg.DatabaseModelPose.deserialize(buffer);
      data.potential_models[i] = tmp.data;
      buffer = tmp.buffer;
    }
    // Deserialize message field [cluster]
    tmp = sensor_msgs.msg.PointCloud.deserialize(buffer);
    data.cluster = tmp.data;
    buffer = tmp.buffer;
    // Deserialize message field [region]
    tmp = SceneRegion.deserialize(buffer);
    data.region = tmp.data;
    buffer = tmp.buffer;
    // Deserialize message field [collision_name]
    tmp = _deserializer.string(buffer);
    data.collision_name = tmp.data;
    buffer = tmp.buffer;
    return {
      data: data,
      buffer: buffer
